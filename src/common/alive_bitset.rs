...
///
/// where `alive_bitset` is the set of alive `RowId`.
...
impl AliveBitSet {
    #[cfg(test)]
    pub(crate) fn for_test_from_deleted_docs(deleted_docs: &[RowId], max_doc: u32) -> AliveBitSet {
    ...
    #[inline]
    pub fn is_alive(&self, doc: RowId) -> bool {
        self.bitset.contains(doc)
    }

    /// Returns true if the document has been marked as deleted.
    #[inline]
    pub fn is_deleted(&self, doc: RowId) -> bool {
        !self.is_alive(doc)
    }

    /// Iterate over the alive doc_ids.
    #[inline]
    pub fn iter_alive(&self) -> impl Iterator<Item = RowId> + '_ {
        self.bitset.iter()
    }
    ...
    pub fn num_alive_svs(&self) -> usize {
        self.num_alive_svs
    }
...
mod tests {
    ...
    fn test_alive_bitset_empty() {
        ...
        assert_eq!(alive_bitset.num_alive_svs(), 10);
    }
    ...
    fn test_alive_bitset() {
        ...
        assert_eq!(alive_bitset.num_alive_svs(), 8);
    }
