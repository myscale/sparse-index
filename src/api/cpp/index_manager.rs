    ...
    if let Err(e) = RAM_BUILDER_CACHE.consume(&index_path, |builder| {
        match index_type {
            InvertedIndexEnum::InvertedIndexImmutableRam => {
                let index =
                    InvertedIndexImmutableRam::from_ram_index(Cow::Owned(builder.build()), index_path.clone()).expect("");
                let _ = index.save(Path::new(&index_path.clone())).expect("can't save immutable ram index file.");
            }
            InvertedIndexEnum::InvertedIndexMmap => {
                // will auto save index file.
                let _ = InvertedIndexMmap::from_ram_index(Cow::Owned(builder.build()), index_path.clone()).expect("can't create mmap index");
            }
            InvertedIndexEnum::InvertedIndexCompressedImmutableRamF32 => {
                let index =
                    InvertedIndexCompressedImmutableRam::<f32>::from_ram_index(Cow::Owned(builder.build()), index_path.clone()).expect("can't create compressed immutable ram index f32");
                let _ = index.save(Path::new(&index_path.clone())).expect("can't save compressed immutable ram file 32.");
            }
            InvertedIndexEnum::InvertedIndexCompressedImmutableRamF16 => {
                let index =
                    InvertedIndexCompressedImmutableRam::<f16>::from_ram_index(Cow::Owned(builder.build()), index_path.clone()).expect("can't create compressed immutable ram index f16");
                let _ = index.save(Path::new(&index_path.clone())).expect("can't save compressed immutable ram file 16.");
            }
            InvertedIndexEnum::InvertedIndexCompressedImmutableRamU8 => {
                let index =
                    InvertedIndexCompressedImmutableRam::<QuantizedU8>::from_ram_index(Cow::Owned(builder.build()), index_path.clone()).expect("can't create compressed immutable ram index u8");
                let _ = index.save(Path::new(&index_path.clone())).expect("can't save compressed immutable ram file u8.");
            }
            InvertedIndexEnum::InvertedIndexCompressedMmapF32 => {
                let index =
                    InvertedIndexCompressedMmap::<f32>::from_ram_index(Cow::Owned(builder.build()), index_path.clone()).expect("can't create compressed mmap index f32");
                let _ = index.save(Path::new(&index_path.clone())).expect("can't save compressed mmap file 32.");
            }
            InvertedIndexEnum::InvertedIndexCompressedMmapF16 => {
                let index =
                    InvertedIndexCompressedMmap::<f16>::from_ram_index(Cow::Owned(builder.build()), index_path.clone()).expect("can't create compressed mmap index f16");
                let _ = index.save(Path::new(&index_path.clone())).expect("can't save compressed mmap file 16.");
            }
            InvertedIndexEnum::InvertedIndexCompressedMmapU8 => {
                let index =
                    InvertedIndexCompressedMmap::<QuantizedU8>::from_ram_index(Cow::Owned(builder.build()), index_path.clone()).expect("can't create compressed mmap index u8");
                let _ = index.save(Path::new(&index_path.clone())).expect("can't save compressed mmap file u8.");
            }
        }
    }) {
        FFIBoolResult {
