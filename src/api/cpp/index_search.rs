...
pub fn ffi_load_index(index_path: &CxxString) -> FFIBoolResult {
    ...
    if let Err(e) = RAM_BUILDER_CACHE.consume(&index_path, |builder| match index_type {
        InvertedIndexEnum::InvertedIndexImmutableRam => {
            let index = InvertedIndexImmutableRam::open(Path::new(&index_path))
        ...
        InvertedIndexEnum::InvertedIndexMmap => {
            let index =
                InvertedIndexMmap::open(Path::new(&index_path)).expect("can't open mmap index");
...
pub fn ffi_sparse_search(
    ...
    // TODO 封装一个自定义的 Flurry Hash Map 简化逻辑
    let search_result = match index_type {
        InvertedIndexEnum::InvertedIndexImmutableRam => {
            let index = InvertedIndexImmutableRam::open(Path::new(&index_path))
                .expect("can't open immutable ram file");
            let pinned = INVERTED_INDEX_IMMUTABLE_RAM_CACHE.pin();
