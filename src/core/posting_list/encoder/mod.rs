...
impl BlockEncoder {
    pub fn new() -> BlockEncoder {
        BlockEncoder { bitpacker: BitPacker4x::new(), output: [0u8; COMPRESSED_BLOCK_MAX_SIZE], output_len: 0 }
    }
    ...
    pub fn compress_block_sorted(&mut self, block: &[u32], offset: u32) -> (u8, &[u8]) {
        ...
        let written_size = self.bitpacker.compress_strictly_sorted(offset, block, &mut self.output[..], num_bits);
...
impl std::fmt::Debug for BlockDecoder {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("BlockDecoder")
            // .field("output", &self.output[..self.output_len])
...
impl BlockDecoder {
    pub fn with_val(val: u32) -> BlockDecoder {
        BlockDecoder { bitpacker: BitPacker4x::new(), output: [val; COMPRESSION_BLOCK_SIZE], output_len: 0 }
    }
    ...
    pub fn uncompress_block_sorted(&mut self, compressed_data: &[u8], offset: u32, num_bits: u8, strict_delta: bool) -> usize {
        if strict_delta {
            let offset = std::num::NonZeroU32::new(offset).map(std::num::NonZeroU32::get);

            self.output_len = COMPRESSION_BLOCK_SIZE;
            self.bitpacker.decompress_strictly_sorted(offset, compressed_data, &mut self.output, num_bits)
        } else {
            self.output_len = COMPRESSION_BLOCK_SIZE;
...
pub trait VIntDecoder {
    ...
    fn uncompress_vint_sorted(&mut self, compressed_data: &[u8], offset: u32, num_els: usize, padding: u32) -> usize;
    ...
    ///
    /// The value given in `padding` will be used to fill the remaining `128 - num_els` values.
    fn uncompress_vint_unsorted(&mut self, compressed_data: &[u8], num_els: usize, padding: u32) -> usize;
...
pub mod tests {
    ...
    use super::*;
    pub const TERMINATED: u32 = i32::MAX as u32;
    ...
    fn test_encode_sorted_block() {
        ...
        {
            let consumed_num_bytes = decoder.uncompress_block_sorted(compressed_data, 0, num_bits, true);
    ...
    fn test_encode_sorted_block_with_offset() {
        ...
        let mut decoder = BlockDecoder::default();
        {
            let consumed_num_bytes = decoder.uncompress_block_sorted(compressed_data, 10, num_bits, true);
            assert_eq!(consumed_num_bytes, compressed_data.len());
        }
        for i in 0..128 {
            assert_eq!(vals[i], decoder.output(i));
    ...
    fn test_encode_sorted_block_with_junk() {
        ...
        let mut decoder = BlockDecoder::default();
        {
            let consumed_num_bytes = decoder.uncompress_block_sorted(&compressed, 10, num_bits, true);
    ...
    fn test_encode_vint() {
        ...
        for offset in &[0u32, 1u32, 2u32] {
            ...
            let mut decoder = BlockDecoder::default();
            let consumed_num_bytes = decoder.uncompress_vint_sorted(encoded_data, *offset, input.len(), PADDING_VALUE);
