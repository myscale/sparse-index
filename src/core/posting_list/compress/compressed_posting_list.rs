...
/// TW 表示真正存储的时候，使用到的 weight 类型，可能会量化，也肯可能没有量化
/// TODO 是否可以使用引用？
...
pub struct CompressedPostingList<TW: QuantizedWeight> {
    ...
    /// Fixed-size chunks.
    /// 当启用 quantized 量化的时候，这个 W 是 u8 否则的话可能是 f32、f16、u8
...
impl<TW: QuantizedWeight> CompressedPostingList<TW> {
    ...
    //     self.storage_size(
    //         |e|
    //         e.blocks.len() * size_of::<CompressedPostingBlock<TW>>()
    //     )
    // }
    ...
    //     self.storage_size(
    //         |e|
    //         e.row_ids_compressed.len() * size_of::<u8>()
    //     )
    // }
}

// impl<'a, W: QuantizedWeight> PostingListTrait<'a, W> for CompressedPostingList<W> {
//     type Iter = CompressedPostingListIterator<'a, W>;
