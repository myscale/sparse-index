...
use crate::{
    core::{
        posting_list::encoder::VIntEncoder, BlockEncoder, DimWeight, ElementRead, ElementType, ElementWrite, ExtendedElement, GenericElement, PostingList, QuantizedParam,
        QuantizedWeight, SimpleElement, WeightType, COMPRESSION_BLOCK_SIZE, DEFAULT_MAX_NEXT_WEIGHT,
    },
    RowId,
};
...
impl<OW: QuantizedWeight, TW: QuantizedWeight> CompressedPostingBuilder<OW, TW> {
    ...
    pub fn pre_build(
        ...
        let mut target_simple_posting_blocks: Vec<SimpleCompressedPostingBlock<TW>> = match self.element_type {
            ElementType::SIMPLE => Vec::with_capacity(self.posting.len() / COMPRESSION_BLOCK_SIZE + 1),
            ElementType::EXTENDED => vec![],
        };
        let mut target_extended_posting_blocks: Vec<ExtendedCompressedPostingBlock<TW>> = match self.element_type {
        ...
        return (row_ids_compressed_in_posting, target_simple_posting_blocks, target_extended_posting_blocks, quantized_param, total_row_ids_count, max_row_id);
    }

    pub fn build(self) -> CompressedPostingList<TW> {
        let element_type = self.element_type;
        let (row_ids_compressed_in_posting, simple_blocks, extended_blocks, quantized_param, total_row_ids_count, max_row_id) = self.pre_build();
...
fn quantized_weights_for_block<OW: QuantizedWeight, TW: QuantizedWeight, F: Fn(&GenericElement<OW>) -> OW>(
    ...
    if quantized_weights.len() > COMPRESSION_BLOCK_SIZE {
        panic!("Expected at most {} elements in a single block, found {}", COMPRESSION_BLOCK_SIZE, quantized_weights.len());
    }
...
fn convert_weights_type_for_block<OW: QuantizedWeight, TW: QuantizedWeight, F: Fn(&GenericElement<OW>) -> OW>(
    ...
    if weights.len() > COMPRESSION_BLOCK_SIZE {
        panic!("Expected at most {} elements in a single block, found {}", COMPRESSION_BLOCK_SIZE, weights.len());
    }
